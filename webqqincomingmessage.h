/*
    webqqincomingmessage.h - Kopete Webqq Protocol
 ...
 */
class WebqqIncomingMessage : public QObject
{
Q_OBJECT
public:
	/**
	 ...
	 */
	WebqqIncomingMessage( WebqqFakeServer* const server , QString message );
	virtual ~WebqqIncomingMessage();
	/**
...
public slots:
	/**
	 * 'Deliver' the message to Kopete by calling WebqqFakeServer::incomingMessage().
	 * This marks the message as delivered so it can be purged from the incoming list.
	 */
	void deliver();
protected:
	QString m_message;
	WebqqFakeServer* m_server;
	bool m_delivered;
};
